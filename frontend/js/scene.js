...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  // 3) Pool throat / wall clip region (yellow)
  const throat = getSpaWallThroatClipBox(poolGroup, spaGroup, 0.01);
  if (throat) {
    const poolTopZ = getPoolCopingTopZ(poolGroup, spaGroup) ?? 0.0;
    const wallVoidDepth = 0.30;
    const minZ = poolTopZ - wallVoidDepth - 0.01;
    const maxZ = Math.max(poolTopZ + 0.01, spaGroup.position.z + Math.max(0.01, spaGroup.userData?.height || 0.01) * 0.5 + 0.01);
    const worldCenter = new THREE.Vector3(
      (throat.minX + throat.maxX) * 0.5,
      (throat.minY + throat.maxY) * 0.5,
      (minZ + maxZ) * 0.5
    );
    addDebugBox(
      group,
      Math.max(0.01, throat.maxX - throat.minX),
      Math.max(0.01, throat.maxY - throat.minY),
  ...
  addDebugBox(group, spaSize.x, spaSize.y, Math.max(0.01, spaSize.z), spaCenter, quat, 0x00ff66, 'SpaBodyDebug');
}
...
function getSpaWallThroatClipBox(poolGroup, spaGroup, pad = 0.01) {
  ...
  // Outside extent now reaches the rebuilt channel coping overhang edge,
  // so the yellow debug/clip box lines up with the channel coping return.
  ...
  if (snapSide === 'left') {
    const channelOuterX = center.x - halfX - (margins.minX || 0);
    minX = Math.min(channelOuterX - pad, bounds.minX - pad);
    maxX = bounds.minX + insidePad;
  } else if (snapSide === 'right') {
    const channelOuterX = center.x + halfX + (margins.maxX || 0);
    minX = bounds.maxX - insidePad;
    maxX = Math.max(channelOuterX + pad, bounds.maxX + pad);
  } else if (snapSide === 'front') {
    const channelOuterY = center.y - halfY - (margins.minY || 0);
    minY = Math.min(channelOuterY - pad, bounds.minY - pad);
    maxY = bounds.minY + insidePad;
  } else if (snapSide === 'back') {
    const channelOuterY = center.y + halfY + (margins.maxY || 0);
    minY = bounds.maxY - insidePad;
    maxY = Math.max(channelOuterY + pad, bounds.maxY + pad);
  } else {
    return null;
  }
