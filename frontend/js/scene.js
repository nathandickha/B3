...
function getAdjacentPoolCopingBounds(poolGroup, spaGroup) {
  ...
  for (const mesh of copingMeshes) {
    ...
    const cp = box.clampPoint(sample.clone(), new THREE.Vector3());
    const distSq = cp.distanceToSquared(sample);
...
export async function initScene() {
  ...
  const skyMat = new THREE.ShaderMaterial({
    ...
    depthWrite: false,
    uniforms: {
      topColor: { value: new THREE.Color(0xeaf6ff) },
...
function getSpaWallThroatClipBox(poolGroup, spaGroup, pad = 0.01) {
  ...
  const alongPad = 0.07 + pad;
  const extraAlongWall = 0.10;
